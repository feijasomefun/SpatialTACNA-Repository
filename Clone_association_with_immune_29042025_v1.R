...
list_of_files = list.files('/Users/arkajyotibhattacharya/Library/CloudStorage/GoogleDrive-a.bhattacharya@rug.nl/My Drive/Projects_27092022/Students/Feija Somefun internship/Manuscript/Main_figures/Fig_6_clonality/Input data/Projection_all_100EV_TCGA_TCs/')

for(sample_id in list_of_files[4:length(list_of_files)])
{
  ...
  current_sample = data.frame(fread(paste0('/Users/arkajyotibhattacharya/Library/CloudStorage/GoogleDrive-a.bhattacharya@rug.nl/My Drive/Projects_27092022/Students/Feija Somefun internship/Manuscript/Main_figures/Fig_6_clonality/Input data/Projection_all_100EV_TCGA_TCs/',sample_id,'/mixing_matrix.tsv')),row.names = 1)
  mix_mat = as.matrix(current_sample)
  mix_mat = t(mix_mat)
  dim(mix_mat)
  ...
  saveRDS(new.seurat.object, paste0('/Users/arkajyotibhattacharya/Library/CloudStorage/GoogleDrive-a.bhattacharya@rug.nl/My Drive/Projects_27092022/Students/Feija Somefun internship/Manuscript/Main_figures/Fig_6_clonality/Results/', sample_id, "_cnb_immune_TCs_cor_between_cnb_immune.rds"))
  ...
  print(plot_grid(plotlist = plot_list_tcs
...
gsea_immune_TCs = gsea_tcga_100ev_immune_genesets
colnames(gsea_immune_TCs) = sapply(colnames(gsea_immune_TCs), function(x){gsub("consensus.independent.component.", "TC",x)})

gsea_immune_TCs = gsea_immune_TCs[,immune_TCs]

gsea_immune_TCs_high_enriched_genesets= apply(gsea_immune_TCs,2, function(x){
  genesets_with_high_enrichment = which(x >5)
  
  genesets_with_high_enrichment_names = paste(rownames(gsea_immune_TCs)[genesets_with_high_enrichment], collapse = " /// ")
  
  
})
gsea_immune_TCs_high_enriched_genesets = t(gsea_immune_TCs_high_enriched_genesets)
gsea_immune_TCs_high_enriched_genesets = t(gsea_immune_TCs_high_enriched_genesets)

write.table(gsea_immune_TCs_high_enriched_genesets, file = "/Users/arkajyotibhattacharya/Library/CloudStorage/GoogleDrive-a.bhattacharya@rug.nl/My Drive/Projects_27092022/Students/Feija Somefun internship/Manuscript/Main_figures/Fig_6_clonality/Results/gsea_immune_TCs_high_enriched_genesets.txt", sep = "\t", quote = FALSE)
