# H5 OBJECT BASE FUNCTION FOR GENERATING RDS FILES AND PC1 REMOVED 
f_remove_PC1_form_RDS <- function(
  ...
  for(current_sample_id in samples_id) {
    ...
    #current_sample_id <- "GSM7179596"
    # ensure all files are loaded since GSM is used anyway 
    ...
    # GSM is start of pattern end is filename
    h5_file_path <- f_pathfinder(current_sample_id,filename = name_h5_object,sample_files, samples_dir, return_name = FALSE)
    mapping_file.path <- f_pathfinder(current_sample_id,filename = name_tissue_position,sample_files, samples_dir,return_name = FALSE)
    print(mapping_file.path)

    ### SCRIPT adjustments for H5 object 
    # use Read10x to load as seurat object
    data <- Read10X_h5(h5_file_path)
    spatial_data <- CreateSeuratObject(data, assay = "Spatial", project = current_sample_id)
    
    # load in features 
    features.path <- "/matrix/features" 
    features <- h5read(h5_file_path, features.path)
    # feature name load in gene ano
    feature.names <- features[["name"]]
    
    #some had hg38 added to feature names had to remove
    feature.names <- gsub("hg38_", "", feature.names)

    # load barcodes 
    barcodes.path <- "/matrix/barcodes"
    barcode.names <- h5read(h5_file_path, barcodes.path)
    
    # Create mapping file: use tissue position 
    # this file is for knowing where each barcode (row and column) is located
    
    mapping_file <- read.csv(mapping_file.path, header = FALSE, row.names = 1)
    ...
    mapping_file <- read.csv(mapping_file.path, header = header_present, row.names = 1)
    ...
    raw_counts <- as.matrix(spatial_data@assays[["Spatial"]]@layers[["counts"]])    
    mat_filtered <-  as.matrix(spatial_data@assays[["Spatial"]]@layers[["counts"]])    
    ...
    # column names of raw count is barcodes
    colnames(raw_counts) <- barcode.names
    ...
    # row names of raw count becomes H5 object features names which is (hugo) gene symbols
    rownames(raw_counts) <- feature.names
    ...
    # probe barcodes that are both in barcodes file and mapping file
    common_samples = intersect(rownames(mapping_file), barcode.names)
    ...
    # check dimensions of raw counts 
    ...
    #1 Perform PCA, remove PC1 export final ncbi matrix
    ...
    # exports as tsv file 
    print(paste0("final matrix: ", nrow(raw_counts_pc1removed)," samples x ",ncol(raw_counts_pc1removed)," genes"))
    ...
    # for those that match find the equivelant NCBI ID 
    ...
    #2 Create, plot and export Seurat object
    # the old seurat object is missing image tissue position etc
    # leave in old code for consistency and removal of NAs 
    ...
    # convert any image to be called lowres 
    ...
    if(grepl("hires", full_image_name) == TRUE )
    {
      ...
      # times by the low scale to remove low scale
      ...
      # set the hires scale factor to be lowres to make sure image gets scaled right
