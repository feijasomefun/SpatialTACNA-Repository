...
####
name_samples_folder <- "25_variety_cancers"
...
# select list of GSM tar files 
sample_files <- list.files(path = samples_dir, pattern = "GSM.*\\.tar$", full.names = TRUE) # mention untar or unzip 
...
f_zip_extraction(sample_files,samples_dir, unzip_function_name = untar) # mention untar or unzip 
...
name_barcodes <- "_barcodes.tsv"
name_features <- "_features.tsv"
name_matrix <- "_matrix.mtx"
...
RDS_function_dir = here("Code", "f_process_into_RDS_3c.R")
