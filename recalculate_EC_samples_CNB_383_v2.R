...
f_generate_CNB <- function(
  ...
  for (sample_dir in projected_samples_dir){
    ...
    # spot_size <- c(
    #   GSM7211257 = 2.3, GSM7782699 = 2, GSM7850822 = 2.1,
    #   CC1 = 1.7, CRC2 = 1.8, EC8 = 1.4, GSM5420752 = 1.6,
    #   GSM5494478 = 2, GSM5833536 = 1.4, GSM6506115 = 1.9,
    #   GSM7498812 = 1.3, GSM7990477 = 1.4, LC2 = 1.7, M1 = 1.4
    # )
    
    # # Get pt.size.factor with fallback
    # spot_size <- ifelse(GSM_id %in% names(spot_size),
    #                   spot_size[[GSM_id]],
    #                   1.6)
    # print(spot_size)
    
    # Get spot.radius from the image
    #spot_radius <- visium@images[["image"]]@spot.radius
    
    # Not working 
    #Number of spots 
    # dim_spots <- visium@assays[["Spatial"]]@layers[["counts"]]@Dim
    # print(dim_spots)
    # number_of_spots <- (dim_spots[2])
    # print(number_of_spots)
    # 
    # # size of image 
    # dim_image <- dim(visium@images[["image"]]@image)
    # image_area <- dim_image[1]*dim_image[2]
    # print(image_area)
    # 
    # average_spot_area <- image_area / number_of_spots
    # #average_spot_area <- average_spot_area*10
    # spot_radius <- (sqrt(average_spot_area / pi))
    # print(spot_radius)
    
    # Ensure pt.size.factor stays within a reasonable range, say between 0.5 and 5
    # spot_size <- max(min(adaptive_pt_size, 5), 0.5)  # Capping between 0.5 and 5
    ...
    CNA_Burden_plot <- SpatialFeaturePlot(new.seurat.object
                                          ,slot = "counts"
                                          ,features = "CNB-383"
                                          ,pt.size = spot_size) + #,pt.size.factor = spot_size)+
